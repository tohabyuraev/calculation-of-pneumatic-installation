...
class EulerianGrid(object):
    def __init__(self, init_data):
        ...
        self.energy_cell = np.full(init_data['num_coor'], self.press_cell / (self.k - 1) / init_data['ro'])
        self.c_cell = np.full(init_data['num_coor'], np.sqrt(self.k * self.press_cell / self.ro_cell))
        ...
        # Первая ячейка является нулевой и их 299
        # Границы располагаются справа и их 299
    ...
    def get_mah_interface(self):
        self.mah_interface = fetta(self.mah_cell_m, self.num_coor, 'plus') + fetta(self.mah_cell_p,
                                                                                   self.num_coor, 'mines')

    def get_press_interface(self):
        self.press_interface = getta(self.mah_cell_m, self.num_coor, 'plus') * self.press_cell + \
                               getta(self.mah_cell_p, self.num_coor, 'mines') * np.roll(self.press_cell, 1)

    def get_tau(self):
        # for i in range(self.num_coor - 1):
        # buff = ((self.x_interface[i + 1] - self.x_interface[i]) / (abs(self.v_interface[i]) + self.c_cell))
        buff = (np.roll(self.x_interface, -1) - self.x_interface) / (abs(self.v_interface) + self.c_cell)
        buff = min(buff)
        self.tau = self.kurant * buff
        # print(np.roll(self.x_interface, 1))
        return self.tau
...
while layer.x_interface[layer.num_coor - 1] <= init_data['L']:
    ...
    answer = sp_cr(layer.press_cell[layer.num_coor - 1], init_data['atmo'], init_data['mass'],
                   layer.v_interface[layer.num_coor - 1], layer.get_tau())
    # print(len(layer.press_cell))
    ...
    get_all_value(layer.get_tau(), all_time_arr, 'time')
