"""
core.py - module module containing the class 'EulerianGrid'
"""
...
class EulerianGrid(object):
    ...
    (пневматика) в газодинамической постановке на подвижной сетке по
    методу Эйлера

    Parameters
    ---
    init_data: dict
        Словарь с начальными данными хар-к АО

    Keyword arguments
    ---
    pressInit: float, optional
    ...
    def __init__(self, init_data, **kwargrs):
        ...
        self.press = init_data.get(
            'pressInit', kwargrs.get(
                'pressInit', self.defaultChar['pressInit']))
        self.ro = init_data.get('ro', self.defaultChar['ro'])
        self.kurant = init_data.get('Ku', self.defaultChar['Ku'])
        self.shellMass = init_data.get(
            'shellMass', self.defaultChar['shellMass'])
        self.tau = 0
        # длина ячейки на пред. шаге (коор-та 1 границы)
        #   необходимо для расчета веторов q
        self.x_prev = 0
        self.S = np.pi * init_data['d'] ** 2 / 4
        self.L0 = init_data.get(
            'L0', kwargrs.get('L0', self.defaultChar['L0']))
        self.L = init_data.get('L', kwargrs.get('L', self.defaultChar['L']))
        ...
        self.energy_cell = np.full(
            self.num_coor, self.press / (self.k - 1) / self.ro)
        self.c_cell = np.full(
            self.num_coor, np.sqrt(self.k * self.press / self.ro))
    ...
    def _border(self):
        ...
        self.q_param[1][self.num_coor - 1] = \
            self.q_param[0][self.num_coor - 1] \
            * (2 * self.v_interface[self.num_coor - 2]
               - self.v_cell[self.num_coor - 2])
    ...
    def getta_mines(self):
        ...
        answer[if_cond] = (self.mah_cell_p[if_cond] - np.abs(self.mah_cell_p[if_cond])) \
            / 2 / self.mah_cell_p[if_cond]
...
if __name__ == '__main__':
    print('Error: попытка запустить core.py')
